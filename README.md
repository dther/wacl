...
- `wacl-minimal.js` and `wacl-minimal.wasm` are your artefacts
  (gitignored — regenerated by the build, not committed)

`make surftcl-demo` builds the wasm and copies the demo site (pages, wasm,
package and test sources) into the sibling `surftcl-demo` repo — the
separate GitHub Pages repository, regenerated from this tree.
